...
function getCasualPixel(lx, ly, direction, walkStep, isTaller, capColor, hairColor) {
    // If taller (Braian), we shift the head and torso UP by 2 pixels, making him taller!
    const tShift = isTaller ? 2 : 0;

    // --- CAP (y: 4 to 9, x: 8 to 23) ---
    const capYStart = 4 - tShift;
    const capYEnd = 9 - tShift;
    if (ly >= capYStart && ly <= capYEnd && lx >= 8 && lx <= 23) {
        // Cap dome
        if (lx >= 10 && lx <= 22 && ly <= capYEnd - 1) {
            if (lx === 10 || lx === 22 || ly === capYStart) return [capColor[0] - 40, capColor[1] - 40, capColor[2] - 40, 255]; // cap outline
            return capColor;
        }
        // Cap Visor (facing direction)
        if (ly === capYEnd) {
            ...
            if (direction === 'up' && lx >= 11 && lx <= 21) return [capColor[0] - 40, capColor[1] - 40, capColor[2] - 40, 255]; // back strap
        }
    }

    // --- HEAD & HAIR (y: 10 to 20, x: 8 to 23) ---
    const headYStart = 10 - tShift;
    const headYEnd = 20 - tShift;
    if (ly >= headYStart && ly <= headYEnd && lx >= 8 && lx <= 23) {
        ...
        if (direction === 'up') {
            if (lx === 8 || lx === 23 || ly === headYEnd) return HAIR_OUTLINE = [45, 20, 10, 255];
            return hairColor;
        }
        ...
        if (lx >= 9 && lx <= 22 && ly <= headYEnd - 1) {
            ...
            // Eyes
            if (ly === headYStart + 5 || ly === headYStart + 6) {
                if (direction === 'down') {
            ...
            if (ly >= headYStart + 9 || lx >= 19) return SKIN_SHADOW;
            return SKIN_BASE;
        }
    }

    // --- BLACK T-SHIRT (Torso, y: 20 to 35) ---
    const torsoYStart = 20 - tShift;
    const torsoYEnd = 35 - tShift;
    ...
    // --- SLEEVES & ARMS ---
    const sleeveYStart = 21 - tShift;
    const sleeveYEnd = 27 - tShift;
    ...
    if (ly >= 45 && ly <= 47 && lx >= leftLegXStart && lx <= leftLegXEnd - 1) {
        if (ly <= leftLegYMax + 5) {
            ...
            if (ly === 46) return SNEAKER_RED; // red stripe
            return SNEAKER_BASE;
        }
    }
    // Right Sneaker
...
// Generate Martin Casual Spritesheet (128x192)
const martinCasualPng = createPng(128, 192, (x, y) => {
    ...
    return getCasualPixel(lx, ly, direction, walkStep, false, [220, 40, 40, 255], [90, 50, 30, 255]); // Red cap, brown hair
});

// Generate Braian Casual Spritesheet (128x192)
const braianCasualPng = createPng(128, 192, (x, y) => {
    const col = Math.floor(x / 32);
    const row = Math.floor(y / 48);
    const lx = x % 32;
    const ly = y % 48;
    ...
    return getCasualPixel(lx, ly, direction, walkStep, true, [30, 30, 35, 255], [120, 30, 30, 255]); // Black cap, red hair, taller!
});

fs.writeFileSync('frontend/public/assets/player/martin_casual.png', martinCasualPng);
fs.writeFileSync('frontend/public/assets/player/braian_casual.png', braianCasualPng);
console.log('Successfully generated casual outfits for Martin and Braian!');

...
const HAIR_BASE = [120, 30, 30, 255]; // Elias has red/ginger hair!
const HAIR_HIGHLIGHT = [180, 70, 70, 255];
const HAIR_OUTLINE = [60, 10, 10, 255];
...
const RED_BELT_BASE = [220, 40, 40, 255];
const RED_BELT_HIGHLIGHT = [255, 120, 120, 255];
const RED_BELT_OUTLINE = [120, 15, 15, 255];
...
function getEliasPixel(lx, ly, direction, walkStep) {
    ...
    if (ly >= beltYStart && ly <= beltYEnd && lx >= 8 && lx <= 23) {
        if (lx === 8 || lx === 23 || ly === beltYStart) return RED_BELT_OUTLINE;
        if (ly === beltYStart + 1 && (lx === 11 || lx === 12 || lx === 19 || lx === 20)) return RED_BELT_HIGHLIGHT;
        return RED_BELT_BASE;
    }
    if (direction === 'down') {
        if (ly === beltYEnd + 1 && lx >= 14 && lx <= 17) {
            if (lx === 15) return RED_BELT_HIGHLIGHT;
            return RED_BELT_BASE;
        }
        if (ly >= beltYEnd + 2 && ly <= beltYEnd + 6 && lx === 14) {
            if (ly === beltYEnd + 6) return RED_BELT_OUTLINE;
            return RED_BELT_BASE;
        }
        if (ly >= beltYEnd + 2 && ly <= beltYEnd + 5 && lx === 16) {
            if (ly === beltYEnd + 5) return RED_BELT_OUTLINE;
            return RED_BELT_BASE;
        }
    }
    ...
    // --- LEGS & FEET ---
    if (ly >= 36 && ly <= 43 && lx >= 8 && lx <= 14) {
        if (lx === 8 || lx === 14 || ly === 43) return SUIT_OUTLINE;
        return SUIT_BASE;
    }
    if (ly >= 36 && ly <= 43 && lx >= 17 && lx <= 23) {
        if (lx === 17 || lx === 23 || ly === 43) return SUIT_OUTLINE;
        return SUIT_SHADOW;
    }
    if (ly >= 44 && ly <= 47 && lx >= 8 && lx <= 13) {
        if (lx === 8 || lx === 13 || ly === 47) return SKIN_OUTLINE;
        return SKIN_BASE;
    }
    if (ly >= 44 && ly <= 47 && lx >= 18 && lx <= 23) {
        if (lx === 18 || lx === 23 || ly === 47) return SKIN_OUTLINE;
        return SKIN_SHADOW;
    }
...
// Generate Elias Spritesheet (128x192)
const eliasPng = createPng(128, 192, (x, y) => {
    ...
    return getEliasPixel(lx, ly, direction, walkStep);
});

fs.writeFileSync('frontend/public/assets/player/guard.png', eliasPng);
console.log('Successfully generated 4-direction spritesheet for Elias!');
